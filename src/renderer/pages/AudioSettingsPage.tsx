...
function AudioSettingsPage() {
  ...
  // 监听来自主进程的播放请求
  useEffect(() => {
    const unsubscribe = window.electronAPI?.onPlayAudioFromHotkey?.((payload) => {
      const { hotkey, filePath } = payload;
      // 找到对应的音效并播放
      const effect = soundEffects.find(e => e.hotkey === hotkey && e.filePath === filePath);
      if (effect) {
    ...
    <div className="h-full p-3 bg-gray-900 text-white">
      ...
      >
        {soundEffects.map((effect) => (
          <div
            key={effect.id}
            className={`bg-gray-800 rounded-lg p-2.5 cursor-pointer hover:bg-gray-700 relative border border-gray-700 transition-all duration-200 ${
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-gray-800 rounded-lg p-4 w-96 max-w-full">
        ...
        <div className="mb-3">
          <div className="mb-3">
            <div className={`p-4 bg-gray-700 text-white rounded border text-center ${
          ...
          <div className="text-xs text-gray-400 text-center">
            <div>💡 支持：F1-F12, Ctrl+A, Alt+F1, Shift+Space 等</div>
          </div>
        </div>
