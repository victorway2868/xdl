...
export class DyCast {
  ...
  private async handleMessage(data: ArrayBuffer) {
    ...
    if (needAck) {
      ...
      if (this.ws && this.ws.readyState === WebSocket.OPEN) {
        ...
        // 重连
        console.error(`ACK发送异常 => 直播间[${this.roomNum}]已关闭`);
        this._afterClose();
        // this.reconnect();
      }
    }
  ...
  private ping() {
    try {
      ...
      // 发送过程出错
      console.error('DyCast Ping Error =>', err);
    }
  }
  ...
  private cannotReceiveMessage() {
    ...
    console.error(`DyCast Cannot Receive Message => after ${tmp} ms`);
    // 重连
